...
function migrateDB() {
    ...
    let migrated = false;
    db.videos.forEach(v => { 
        if (!v.comments) { v.comments = []; migrated = true; } 
        if (v.views === undefined) { v.views = 0; migrated = true; }
        if (v.description === undefined) { v.description = ''; migrated = true; }
    });
    db.users.forEach(u => { 
        if (!u.avatar || u.avatar === '/default-avatar.png') { u.avatar = DEFAULT_AVATAR; migrated = true; } 
        if (!u.subscribers) { u.subscribers = []; migrated = true; }
        if (!u.subscribedTo) { u.subscribedTo = []; migrated = true; }
    });
...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => { cb(null, 'chunk-' + Date.now() + path.extname(file.originalname)); }
});
...
app.post('/api/signup', (req, res) => {
    ...
    if (!username || !password || username.trim() === "") return res.status(400).json({ success: false, message: 'Valid username and password are required.' });
    const db = readDB();
    if (db.users.find(u => u.username.toLowerCase() === username.trim().toLowerCase())) return res.status(400).json({ success: false, message: 'Username already exists.' });
    
    db.users.push({ username: username.trim(), password, avatar: DEFAULT_AVATAR, subscribers: [], subscribedTo: [] });
    writeDB(db);
    res.json({ success: true, message: 'Account created!', username: username.trim() });
});
...
app.get('/api/users/:username', (req, res) => {
    ...
    if (user) {
        res.json({ success: true, username: user.username, avatar: user.avatar, subscribers: user.subscribers, subscribedTo: user.subscribedTo });
...
app.put('/api/users/password', (req, res) => {
    ...
    } else res.status(401).json({ success: false, message: 'Incorrect old password.' });
});
...
app.post('/api/users/:targetUser/subscribe', (req, res) => {
    ...
    if (!currentUser || currentUser === targetUsername) return res.status(400).json({ success: false });
    ...
    if (target && user) {
        ...
        if (subIndex === -1) {
            ...
            user.subscribedTo = user.subscribedTo.filter(name => name !== targetUsername);
        }
        writeDB(db);
        res.json({ success: true, subscribersCount: target.subscribers.length, isSubscribed: subIndex === -1 });
    } else res.status(404).json({ success: false });
});
...
app.post('/api/upload', upload.single('video'), (req, res) => {
    if (!req.file) return res.status(400).json({ success: false, message: 'No chunk provided' });
    
    const { taskId, chunkIndex, totalChunks, username, title, description } = req.body;
...
function startFFmpegProcessing(taskId, rawPath, username, title, description) {
    ...
    processingTasks[taskId] = { 
        ...
        videoData: {
            id: taskId, uploader: username || 'Anonymous',
...
app.get('/api/videos/top', (req, res) => {
    const db = readDB();
    const sorted = [...db.videos].sort((a, b) => (b.views || 0) - (a.views || 0)).slice(0, 3);
    res.json(sorted);
});
...
app.get('/api/search', (req, res) => {
    ...
    const db = readDB();
    const results = db.videos.filter(v => v.title.toLowerCase().includes(query) || v.uploader.toLowerCase().includes(query));
    res.json(results);
});
...
app.get('/api/channels/search', (req, res) => {
    const query = req.query.q ? req.query.q.toLowerCase() : '';
    const db = readDB();
    const results = db.users.filter(u => u.username.toLowerCase().includes(query)).map(u => ({ username: u.username, avatar: u.avatar }));
    res.json(results);
});
...
app.put('/api/videos/:id', (req, res) => {
    ...
    if (videoIndex > -1 && db.videos[videoIndex].uploader === username) {
        db.videos[videoIndex].title = title; writeDB(db); res.json({ success: true });
...
app.delete('/api/videos/:id', (req, res) => {
    ...
    if (videoIndex > -1 && db.videos[videoIndex].uploader === username) {
        ...
        const thumbPath = video.thumbnailUrl ? path.join(__dirname, video.thumbnailUrl) : null;
        
        if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
        if (thumbPath && fs.existsSync(thumbPath)) fs.unlinkSync(thumbPath);

        db.videos.splice(videoIndex, 1); writeDB(db); res.json({ success: true });
    } else res.status(403).json({ success: false, message: 'Unauthorized' });
});
...
app.post('/api/videos/:id/like', (req, res) => {
    ...
    if (video) {
        ...
        writeDB(db); res.json({ success: true, likes: video.likedBy.length });
    }
});

app.post('/api/videos/:id/comments', (req, res) => {
    const { username, text } = req.body;
    if (!username || !text.trim()) return res.status(400).json({ success: false });
    const db = readDB();
    const user = db.users.find(u => u.username === username);
    const video = db.videos.find(v => v.id === req.params.id);
    if (video && user) {
        if (!video.comments) video.comments = [];
        const newComment = { id: Date.now().toString(), username, avatar: user.avatar, text: text.trim(), date: new Date().toLocaleDateString() };
        video.comments.push(newComment); writeDB(db); res.json({ success: true, comments: video.comments });
    } else res.status(404).json({ success: false });
});

app.listen(PORT, HOST, () => { console.log(`454 Videos V3.1.0 running on http://${HOST}:${PORT}`); });